package com.vincent.videocompressor.activity;
...
import com.vincent.videocompressor.VideoCompressorUtil;
import com.vincent.videocompressor.adapter.GridImageAdapter;
import com.vincent.videocompressor.listener.DragListener;
import com.vincent.videocompressor.pictureselector.FullyGridLayoutManager;
import com.vincent.videocompressor.pictureselector.GlideCacheEngine;
import com.vincent.videocompressor.pictureselector.GlideEngine;
...
public class VideoComPressorActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    private TextView tv_VideoCompress, tv_VideoCompress1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_video_com_pressor);
        ...
        tv_VideoCompress = findViewById(R.id.tv_VideoCompress);
        tv_VideoCompress1 = findViewById(R.id.tv_VideoCompress1);
        ...
        tv_VideoCompress.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        videoImage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (video_Paths != null && !"".equals(video_Paths)) {
                    PictureSelector.create(VideoComPressorActivity.this)
                            .themeStyle(com.luck.picture.lib.R.style.picture_default_style)
                            .setPictureStyle(mPictureParameterStyle)// 动态自定义相册主题
                            .externalPictureVideo(video_Paths);
                }
            }
        });
...
//                        animationStyle.activityPreviewExitAnimation = R.anim.picture_anim_down_out;
    ...
    private boolean isVideoCompressing = false;//视频是否正在压缩
    private String url;//视频的本地地址

    /**
     * @param inputPath 视频原地址
     * @param outPath   视频压缩后的地址
     */
    public void compressVideo2(String inputPath, String outPath) {
        File file1 = new File(inputPath);
        if (!file1.exists()) {
            com.blankj.utilcode.util.ToastUtils.showShort("文件不存在，请重新选择");
            return;
        }
        ...
        VideoCompress.compressVideoMedium(inputPath, outPath, new VideoCompress.CompressListener() {
            @Override
            public void onStart() {
//                tv_indicator.setText("压缩中..." + "\n"
//                        + "开始时间: " + new SimpleDateFormat("HH:mm:ss", VideoCompressorUtil.getLocale()).format(new Date()));
////                        tv_proess.setVisibility(View.VISIBLE);
//                startTime = System.currentTimeMillis();
                VideoCompressorUtil.writeFile(VideoComPressorActivity.this, "Start at: " + new SimpleDateFormat("HH:mm:ss", VideoCompressorUtil.getLocale()).format(new Date()) + "\n");
...
//                tv_VideoCompress.setText("成功");
//                endTime = System.currentTimeMillis();
                try {
                    tv_VideoCompress.setText(VideoCompressorUtil.getFileSize(new File(inputPath)) + "M" + "\n"
                            + "压缩成功!" + "\n"
                            + "结束时间: " + new SimpleDateFormat("HH:mm:ss", VideoCompressorUtil.getLocale()).format(new Date()));
                    tv_VideoCompress1.setText("压缩后大小：" + VideoCompressorUtil.getFileSize(new File(outPath)) + "M");
                } catch (Exception e) {
                    e.printStackTrace();
                }
          /*              VideoCompressorUtil.writeFile(MainActivity.this, "End at: " + new SimpleDateFormat("HH:mm:ss", getLocale()).format(new Date()) + "\n");
                        VideoCompressorUtil.writeFile(MainActivity.this, "Total: " + ((endTime - startTime) / 1000) + "s" + "\n");
                        VideoCompressorUtil.writeFile(MainActivity.this);*/
//                videoImage.setVisibility(View.VISIBLE);
//                videoDesc.setVisibility(View.VISIBLE);
                compressionMsg.setVisibility(View.GONE);
//                Drawable top = getResources().getDrawable(R.drawable.ic_play_circle_outline_white_48dp);
//                videoDesc.setCompoundDrawablesWithIntrinsicBounds(null, top, null, null);
                url = outPath;
...
//                    Bitmap bitmap2 = ThumbnailUtils.createVideoThumbnail(file.getAbsolutePath(), MediaStore.Video.Thumbnails.FULL_SCREEN_KIND);
//                GlideEngine.createGlideEngine().loadImage(VideoComPressorActivity.this, file.getAbsolutePath(), videoImage);
//                    videoImage.setImageBitmap(bitmap2);
//                }
            }
...
//                endTime = System.currentTimeMillis();
                VideoCompressorUtil.writeFile(VideoComPressorActivity.this, "Failed Compress!!!" + new SimpleDateFormat("HH:mm:ss", VideoCompressorUtil.getLocale()).format(new Date()));
            }
...
//                tv_proess.setText(String.valueOf(percent) + "%");
                ...
                progressTime.setText("视频压缩中:" + new DecimalFormat(".00").format(percent) + "%");
     ...
     * 把bitmap保存到本地
     */
    private static File saveBitmapToBeLocalFile(Bitmap bm, String path, String fileName) {
    ...
    private static class MyCustomPreviewInterfaceListener implements OnCustomImagePreviewCallback<LocalMedia> {
        ...
        public void onCustomPreviewCallback(Context context, List<LocalMedia> previewData, int currentPosition) {
            // TODO context特指VideoComPressorActivityivity
        ...
        if (requestCode == PictureConfig.APPLY_STORAGE_PERMISSIONS_CODE) {// 存储权限
            for (int grantResult : grantResults) {
                if (grantResult == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(VideoComPressorActivity.this,
