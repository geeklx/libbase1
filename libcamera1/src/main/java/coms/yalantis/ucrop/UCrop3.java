...
public class UCrop3 {
     ...
     */
    public static UCrop3 of(@NonNull String intent, @NonNull Uri source, @NonNull Uri destination, ArrayList<String> totalSource) {
        if (totalSource == null || totalSource.size() <= 0) {
            throw new IllegalArgumentException("Missing required parameters, count cannot be less than 1");
        }
        //
        String intent1 = intent;
        if (totalSource != null && totalSource.size() > 1) {
            intent1 = AppUtils.getAppPackageName() + ".hs.act.slbapp.UCropMultipleActivity";
        } else {
            intent1 = intent;
        }
        //
        if (totalSource.size() == 1) {
            return new UCrop3(intent1, source, destination);
        }
        return new UCrop3(intent1, source, destination, totalSource);
    }
     ...
     * @param destination Uri for saving the cropped image
     */
    public static <T> UCrop3 of(@NonNull String intent, @NonNull Uri source, @NonNull Uri destination) {
        return new UCrop3(intent, source, destination);
    }
     ...
     * @param destination Uri for saving the cropped image
     */
    private UCrop3(String intent, @NonNull Uri source, @NonNull Uri destination) {
        mCropIntent = new Intent(intent);
     ...
     */
    private UCrop3(String intent, @NonNull Uri source, @NonNull Uri destination, ArrayList<String> totalSource) {
        mCropIntent = new Intent(intent);
        mCropOptionsBundle = new Bundle();
        mCropOptionsBundle.putParcelable(EXTRA_INPUT_URI, source);
        mCropOptionsBundle.putParcelable(EXTRA_OUTPUT_URI, destination);
    ...
    public void setImageEngine(UCropImageEngine engine) {
        ...
        boolean isUseBitmap = mCropOptionsBundle.getBoolean(UCrop3.Options.EXTRA_CROP_CUSTOM_LOADER_BITMAP, false);
     ...
     */
    public UCrop3 withAspectRatio(float x, float y) {
     ...
     * User won't see the menu with other ratios options.
     */
    public UCrop3 useSourceImageAspectRatio() {
     ...
     */
    public UCrop3 withMaxResultSize(@IntRange(from = MIN_SIZE) int width, @IntRange(from = MIN_SIZE) int height) {
        if (width < MIN_SIZE) {
            width = MIN_SIZE;
        }
    ...
    public UCrop3 withOptions(@NonNull Options options) {
...
//        }
    ...
    public static Uri getOutput(@NonNull Intent intent) {
        return intent.getParcelableExtra(UCrop3.EXTRA_OUTPUT_URI);
    }
    ...
    public static class Options {
         ...
         */
        public void setAllowedGestures(@UCropActivity.GestureTypes int tabScale, @UCropActivity.GestureTypes int tabRotate, @UCropActivity.GestureTypes int tabAspectRatio) {
        ...
        public void setAspectRatioOptions(int selectedByDefault, AspectRatio... aspectRatio) {
            if (selectedByDefault >= aspectRatio.length) {
                throw new IllegalArgumentException(String.format(Locale.US, "Index [selectedByDefault = %d] (0-based) cannot be higher or equal than aspect ratio options count [count = %d].", selectedByDefault, aspectRatio.length));
            }
