...
public class CameraProxy implements Camera.AutoFocusCallback {
    ...
    private void initConfig() {
        ...
        try {
            ...
            if (supportedFocusModes != null && supportedFocusModes.contains(Parameters.FOCUS_MODE_AUTO)) {
                ...
                Log.e("CameraProxy", "自动聚焦了");
            }
            //   mParameters.setPreviewFormat(ImageFormat.NV21); // 设置预览图片格式
            mParameters.setPreviewFormat(ImageFormat.JPEG); // 设置预览图片格式
            ...
            mParameters.setPreviewSize(676, 902); // 设置预览图片大小
            ...
            mParameters.setPictureSize(676, 902);
    ...
    public void focusOnPoint(int x, int y, int width, int height) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("CameraProxy", e.toString());
        }
    }

    // handle button auto focus
    public void doAutoFocus1() {
    ...
    public void onAutoFocus(boolean success, Camera camera) {
        ...
        if (success) {
            ...
            if (!Build.MODEL.equals("KORIDY H30")) {
                ...
                mParameters.setFocusMode(Parameters.FOCUS_MODE_CONTINUOUS_PICTURE);// 1连续对焦
                mCamera.setParameters(mParameters);
            } else {
                mParameters = mCamera.getParameters();
